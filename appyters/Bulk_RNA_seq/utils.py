...
def plot_2D_scatter(x, y, text='', title='', xlab='', ylab='', hoverinfo='text', color='black', colorscale='Blues',
    ...
    if return_trace:
        ...
        if de_type == 'ma':
            annotations = [
                {'x': 1, 'y': 0.1,
                 'text': '<span style="color: blue; font-size: 10pt; font-weight: 600;">Down-regulated in ' + labels[
                     -1] + '</span>', 'showarrow': False, 'xref': 'paper', 'yref': 'paper', 'xanchor': 'right',
                 'yanchor': 'top'},
                {'x': 1, 'y': 0.9,
                 'text': '<span style="color: red; font-size: 10pt; font-weight: 600;">Up-regulated in ' + labels[
                     -1] + '</span>', 'showarrow': False, 'xref': 'paper', 'yref': 'paper', 'xanchor': 'right',
                 'yanchor': 'bottom'}
            ] if labels else []
            ...
            annotations = [
                {'x': 0.25, 'y': 1.07,
                 'text': '<span style="color: blue; font-size: 10pt; font-weight: 600;">Down-regulated in ' + labels[
                     -1] + '</span>', 'showarrow': False, 'xref': 'paper', 'yref': 'paper', 'xanchor': 'center'},
                {'x': 0.75, 'y': 1.07,
                 'text': '<span style="color: red; font-size: 10pt; font-weight: 600;">Up-regulated in ' + labels[
                     -1] + '</span>', 'showarrow': False, 'xref': 'paper', 'yref': 'paper', 'xanchor': 'center'}
            ] if labels else []
    ...
    for cls1, cls2 in combinations(classes, 2):
        ...
        if method == "limma":
            ...
            edgeR_results = pandas2ri.conversion.rpy2py(
                edgeR(pandas2ri.conversion.py2rpy(expr_df), pandas2ri.conversion.py2rpy(cls1_sample_ids),
            ...
            DESeq2_results = pandas2ri.conversion.rpy2py(
                DESeq2(pandas2ri.conversion.py2rpy(expr_df), pandas2ri.conversion.py2rpy(cls1_sample_ids),
                       pandas2ri.conversion.py2rpy(cls2_sample_ids)))
...
def run_volcano(signature, signature_label, dataset, pvalue_threshold, logfc_threshold, plot_type):
    ...
    for index, rowData in signature.iterrows():
        ...
        # Text
        text.append('<b>' + index + '</b><br>Avg Expression = ' + str(
            round(rowData[expr_colname], ndigits=2)) + '<br>logFC = ' + str(
            round(rowData[logfc_colname], ndigits=2)) + '<br>p = ' + '{:.2e}'.format(
            rowData[pval_colname]) + '<br>FDR = ' + '{:.2e}'.format(rowData[pval_colname]))
...
def plot_library_barchart(enrichr_results, gene_set_library, signature_label, sort_results_by='pvalue', nr_genesets=15,
    ...
    for i, geneset in enumerate(['upregulated', 'downregulated']):
        ...
        plot_dataframe['overlapping_genes'] = ['<br>'.join([', '.join(genes[i:i + n]) for i in range(0, len(genes), n)])
        ...
        bar = go.Bar(
            ...
            showlegend=False,
            hovertext=[
                '<b>{term_name}</b><br><b>P-value</b>: <i>{pvalue:.2}</i><br><b>FDR</b>: <i>{FDR:.2}</i><br><b>Z-score</b>: <i>{zscore:.3}</i><br><b>Combined score</b>: <i>{combined_score:.3}</i><br><b>{nr_genes} Genes</b>: <i>{overlapping_genes}</i><br>'.format(
    ...
    annotations = [
        {'x': 0.25, 'y': 1.06,
         'text': '<span style="color: #FA8072; font-size: 10pt; font-weight: 600;">Up-regulated in ' +
                 labels[-1] + '</span>', 'showarrow': False, 'xref': 'paper', 'yref': 'paper', 'xanchor': 'center'},
        {'x': 0.75, 'y': 1.06,
         'text': '<span style="color: #87CEFA; font-size: 10pt; font-weight: 600;">Down-regulated in ' +
                 labels[-1] + '</span>', 'showarrow': False, 'xref': 'paper', 'yref': 'paper', 'xanchor': 'center'}
...
def results_table(enrichment_dataframe, source_label, target_label, notebook_metadata, table_counter):
    ...
    for gene_set_library in enrichment_dataframe['gene_set_library'].unique():
        ...
        enrichment_dataframe_subset[source_label] = [
            '<a href="https://www.mirbase.org/cgi-bin/query.pl?terms={}" target="_blank">{}</a>'.format(x.split(" ")[0],
                                                                                                        x) if '-miR-' in x else '<a href="https://maayanlab.cloud/Harmonizome/gene/{}" target="_blank">{}</a>'.format(
        ...
        enrichment_dataframe_subset[source_label] = [
            rowData[source_label].replace('target="_blank">', 'target="_blank"><b>').replace('</a>', '*</b></a>') if
            rowData['FDR'] < 0.05 else rowData[source_label] for index, rowData in
            enrichment_dataframe_subset.iterrows()]

        # Add rank
        enrichment_dataframe_subset['Rank'] = ['<b>' + str(x + 1) + '</b>' for x in
        ...
        html_table = enrichment_dataframe_subset.head(50)[
            ['Rank', source_label, 'P-value', 'FDR', target_label.title()]].to_html(escape=False, index=False,
                                                                                    classes='w-100')
        ...
        table_counter, notebook_metadata = display_object(table_counter, gene_set_library + additional_description,
                                                          notebook_metadata, saved_filename=filename, istable=True)
...
def display_table(analysis_results, source_label, notebook_metadata, table_counter):
    # Plot Table
    return results_table(analysis_results['enrichment_dataframe'].copy(), source_label=source_label,
                         notebook_metadata=notebook_metadata, target_label='target', table_counter=table_counter)
...
def run_l1000cds2(signature, nr_genes=500, signature_label='', plot_type='interactive'):
    ...
    for aggravate in [True, False]:
        ...
        # Send to API
        config = {"aggravate": aggravate, "searchMethod": "geneSet", "share": True, "combination": False,
                  "db-version": "latest"}
        r = requests.post('https://maayanlab.cloud/L1000CDS2/query', data=json.dumps({"data": data, "config": config}),
        ...
        if resGeneSet.get('topMeta'):
            l1000cds2_dataframe = pd.DataFrame(resGeneSet['topMeta'])[
                ['cell_id', 'pert_desc', 'pert_dose', 'pert_dose_unit', 'pert_id', 'pert_time', 'pert_time_unit',
                 'pubchem_id', 'score', 'sig_id']].replace('-666', np.nan)
            l1000cds2_results[label] = {
                'url': 'https://maayanlab.cloud/L1000CDS2/#/result/{}'.format(resGeneSet['shareId']),
                'table': l1000cds2_dataframe}
        else:
...
def plot_l1000cds2(l1000cds2_results, counter, notebook_metadata, nr_drugs=7, height=300, plot_name="L1000CDS2.png"):
    ...
    if not l1000cds2_results['mimic'] or not l1000cds2_results['reverse']:
        ...
        fig['layout'].update(height=height,
                             title='<b>L1000CDS<sup>2</sup> {}| Small Molecule Query</b><br><i>Top small molecules</i>'.format(
                                 l1000cds2_results['signature_label']), hovermode='closest')
...
def run_l1000fwd(signature, nr_genes=500, signature_label=''):
    ...
    # Get Data
    payload = {"up_genes": upperGenes(signature.index[:nr_genes]),
               "down_genes": upperGenes(signature.index[-nr_genes:])}
...
def plot_l1000fwd(l1000fwd_results, figure_counter, table_counter, notebook_metadata, nr_drugs=7, height=300):
    ...
    if l1000fwd_results['result_url']:
        ...
        figure_counter, notebook_metadata = display_object(figure_counter,
                                                           "L1000FWD plot for visualizing top mimickers (red) and top reversers (blue) of {}.".format(
                                                               l1000fwd_results["signature_label"]), notebook_metadata,
                                                           saved_filename=l1000fwd_results['result_url'], istable=False)
        # Display tables
        for direction, signature_list in l1000fwd_results['signatures'].items():
            # Fix dataframe
            rename_dict = {'sig_id': 'Signature ID', 'pvals': 'P-value', 'qvals': 'FDR', 'zscores': 'Z-score',
                           'combined_scores': 'Combined Score'}
            signature_dataframe = pd.DataFrame(signature_list)[list(rename_dict.keys())].rename(
                columns=rename_dict).sort_values('P-value').rename_axis('Rank')
            signature_dataframe.index = [x + 1 for x in range(len(signature_dataframe.index))]
            signature_txt = signature_dataframe.to_csv(sep='\t')

            # Display table
            pd.set_option('max.colwidth', -1)
            signature_dataframe['Signature ID'] = [
                '<a href="https://maayanlab.cloud/dmoa/sig/{x}" target="_blank">{x}</a>'.format(**locals()) for x in
                signature_dataframe['Signature ID']]
            table_html = signature_dataframe.to_html(escape=False, classes='w-100')

            display(HTML(
                '<style>.w-100{{width: 100% !important;}}</style><div style="max-height: 250px; overflow-y: auto; margin-bottom: 25px;">{}</div>'.format(
                    table_html)))
            filename = "{} Signatures for {}.csv".format(direction.title(), l1000fwd_results["signature_label"])
            table_counter, notebook_metadata = display_object(table_counter,
                                                              "L1000FWD Results: {} Signatures of {}".format(
                                                                  direction.title(),
                                                                  l1000fwd_results["signature_label"]),
                                                              notebook_metadata, saved_filename=filename, istable=True)
